...
ssh -l $REMOTE_USER $REMOTE_IP "podman compose --env-file $TMP_FOLDER/.env -f $TMP_FOLDER/stack.yml stop stack_block_divider_core_1"
ssh -l $REMOTE_USER $REMOTE_IP "podman container prune -f"
ssh -l $REMOTE_USER $REMOTE_IP "podman compose --env-file $TMP_FOLDER/.env -f $TMP_FOLDER/stack.yml start stack_block_divider_core_1"
